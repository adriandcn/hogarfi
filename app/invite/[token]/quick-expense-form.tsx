...
export default function QuickExpenseForm({
  ...
  const [categoryIcon, setCategoryIcon] = useState('groceries')

  const categories = [
    { name: 'Comida', icon: 'groceries' },
    { name: 'Servicios', icon: 'services' },
    { name: 'Transporte', icon: 'transport' },
    { name: 'Salud', icon: 'health' },
    { name: 'Otro', icon: 'other' },
  ]
    ...
    try {
      const res = await fetch('/api/expense/quick', {
        ...
        body: JSON.stringify({
          ...
          icon: categoryEmojis[categoryIcon],
        }),
      })
      if (res.ok) {
        setSaved(true)
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', maxWidth: 430, margin: '0 auto' }}>
      ...
      <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 14, paddingBottom: 120 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <div style={{ position: 'relative' }}>
            <span style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', fontFamily: 'var(--font-syne)', fontSize: 22, fontWeight: 700, color: 'var(--ink3)' }}>
              $
            </span>
            <input
              type="number"
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, padding: '16px 20px 30px', background: 'linear-gradient(to top, var(--bg) 70%, transparent)', maxWidth: 430, margin: '0 auto' }}>
        <button
          ...
          style={{ width: '100%', background: 'var(--ink)', color: '#fff', border: 'none', borderRadius: 14, padding: '16px', fontSize: 16, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-syne)', opacity: (!amount || !description || saving) ? 0.5 : 1 }}>
