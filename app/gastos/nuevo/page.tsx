...
export default function NuevoGastoPage() {
  ...
  function getMemberColor(i: number) {
    const colors = [
  ...
  async function handleSave() {
    ...
    let splits: { memberId: string; percentage: number; amount: number }[] = []
    ...
    if (tab === 'uno') {
      ...
      splits = members.map(m => ({
        memberId: m.id,
        percentage: customShares[m.id] ?? 0,
        amount: amountNum * (customShares[m.id] ?? 0) / 100,
      }))
    }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--off)' }}>
      ...
      <div style={{ background: 'var(--title)', padding: '52px 20px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <button onClick={() => router.back()} style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,.1)', border: 'none', color: '#fff', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>←</button>
      ...
      <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 20, paddingBottom: 120 }}>
        ...
        <div>
          ...
          {tab === 'uno' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.5, marginBottom: 4 }}>
                Una persona pago el total. El balance se ajusta automaticamente segun los porcentajes del hogar.
              </div>
              {members.map((m, i) => {
          ...
          {tab === 'default' && (
            <div>
              <div style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.5, marginBottom: 12 }}>
                Cada miembro paga su porcentaje acordado del hogar. Refleja la distribucion justa segun los ingresos de cada uno.
              </div>
          ...
          {tab === 'custom' && (
            <div>
              <div style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.5, marginBottom: 12 }}>
                Ajusta los porcentajes para este gasto especifico. Deben sumar 100%.
              </div>
        ...
        <div>
          <div onClick={() => document.getElementById('receipt-input')?.click()}
            style={{ border: '1.5px dashed var(--border)', borderRadius: 'var(--r-md)', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, cursor: scanning ? 'default' : 'pointer', background: scanning ? 'rgba(201,242,106,.04)' : 'transparent' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--muted)' }}>
                {scanning ? 'Extrayendo monto y descripcion' : 'Opcional — extrae datos automaticamente'}
              </div>
            </div>
          </div>
