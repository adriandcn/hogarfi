      ...
      <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '18px' }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink3)', letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 12 }}>
            Resumen — {monthNames[current.month - 1]} {current.year}
          </div>
              ...
              <div key={i} style={{ padding: '12px 16px', borderTop: i > 0 ? '1px solid var(--border)' : 'none' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    {over && <span style={{ fontSize: 10, background: 'rgba(255,107,74,.15)', color: 'var(--coral)', borderRadius: 4, padding: '2px 6px', fontWeight: 700 }}>⚠ Excedido</span>}
                  </div>
                ...
                <div style={{ height: 5, background: 'var(--surface2)', borderRadius: 99, overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: 99, background: over ? 'var(--coral)' : cat.color, width: `${pct}%`, transition: 'width .6s' }}/>
                </div>
              </div>
            )
          })}
            ...
            {[...data.monthlyData].reverse().map((m, i) => {
              ...
              return (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                  ...
                  <div style={{ width: '100%', height: `${h}%`, background: i === data.monthlyData.length - 1 - activeMonth ? 'var(--ink)' : 'var(--surface2)', borderRadius: '4px 4px 0 0', transition: 'height .6s' }}/>
